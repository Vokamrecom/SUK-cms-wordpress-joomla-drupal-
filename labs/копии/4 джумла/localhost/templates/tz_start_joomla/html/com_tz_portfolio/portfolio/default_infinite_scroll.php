...
<div id="tz_append" class="text-center">
    <?php if ($params->get('tz_portfolio_layout') == 'ajaxButton'): ?>
        <button class="button" id="load-more">
            <?php echo JText::_('TPL_TZ_START_LOAD_MORE_LABEL'); ?> <i class="fa fa-refresh"></i>
        </button>
    <?php endif; ?>
</div>
    ...
    jQuery(function () {
        ...
        $container.infinitescroll({
                ...
                errorCallback: function () {
                    ...
                    jQuery('#tz_append button').unbind('click').html('<?php echo JText::_('COM_TZ_PORTFOLIO_NO_MORE_PAGES');?>').show();
                    <?php endif;?>
                    <?php if($this -> params -> get('tz_portfolio_layout') == 'ajaxInfiScroll'):?>
                    jQuery('#tz_append').removeAttr('style').html('<button class="tzNomore"><?php echo JText::_('COM_TZ_PORTFOLIO_NO_MORE_PAGES');?></button>');
                    <?php endif;?>
                    jQuery('#tz_append button').addClass('tzNomore');
                    jQuery('#infscr-loading').css('opacity', '0');
                }
            ...
            function (newElements) {
                ...
                $scroll = true;
                jQuery('#tz_append button.button i').removeClass('fa-spin');
            }
        );
        ...
        jQuery('div#tz_append button').click(function () {
            jQuery(this).stop();
            jQuery('#tz_append button.button i').addClass('fa-spin');
            $container.infinitescroll('retrieve');
        });
