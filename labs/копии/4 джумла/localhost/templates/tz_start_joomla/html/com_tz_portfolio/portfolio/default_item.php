...
<?php if ($this->listsArticle): ?>
    ...
    <?php foreach ($this->listsArticle as $i => $row): ?>
        ...
        <?php if ((isset($listMedia[0]->type) AND $listMedia[0]->type != 'quote'
            ...
            <?php endif; ?>
            <div class="TzPortfolioDescription cover">
                <div class="cover-inner">
                    <?php if ($params->get('show_title', 1)): ?>
                        <h5 class="TzPortfolioTitle name photo-name">
                            <?php echo $this->escape($row->title); ?>
                        </h5>
                    <?php endif; ?>
                    <?php if ($params->get('show_category', 1)) : ?>
                        <h6 class="photo-cat">
                            <?php echo $title = $this->escape($row->category_title); ?>
                ...
                if ($listMedia[0]->type == 'video'):
                    ...
                    if ($thbSize) :
                        $src = JURI::root().str_replace('.' . JFile::getExt($listMedia[0]->thumb), '_'
                                . $thbSize
                                . '.' . JFile::getExt($listMedia[0]->thumb), $listMedia[0]->thumb);
                    endif;
                else:
                    if ($params->get('portfolio_image_size', 'M')) {
                    ...
                    if ($imgSize) :
                        if (!empty($listMedia[0]->images)):
                            $src = JURI::root().str_replace('.' . JFile::getExt($listMedia[0]->images), '_' . $imgSize
                                . '.' . JFile::getExt($listMedia[0]->images), $listMedia[0]->images);
                        endif;
                    endif;
                endif;
                ?>
				   <a class="zoom" data-placement="bottom" data-toggle="tooltip"
                               data-original-title="View item" title="" rel="prettyPhoto" href="<?php echo $src;?>">
                                +
                            </a>
                ...
                <?php if (($params->get('show_author', 1))  or ($params->get('show_create_date', 1)) or ($params->get('show_modify_date', 1)) or ($params->get('show_publish_date', 1)) or ($params->get('show_parent_category', 1)) or ($params->get('show_hits', 1))) : ?>
