...
class Com_EasyFrontendSeoInstallerScript
{
     ...
     *
     * @param string $versionMin    Minimum required Joomla! version - e.g. 3.2
     * @param string $extensionName Name of the extension of the warning message
     *
     * @return bool
     * @throws Exception
     */
    private function checkVersionRequirements($versionMin, $extensionName)
    {
        $version = new JVersion();

        if (!$version->isCompatible($versionMin)) {
            JFactory::getApplication()->enqueueMessage(JText::sprintf('KR_JOOMLA_VERSION_REQUIREMENTS_NOT_MET_INSTALLATION', $extensionName, $versionMin), 'error');
    ...
    function postflight($type, $parent)
    {
        ...
        $columnFirst = $db->loadAssoc();

        if ($columnFirst['Field'] !== 'id') {
