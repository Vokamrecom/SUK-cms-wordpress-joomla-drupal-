...
class EasyFrontendSeoHelper
{
    ...
    public static function getDonationCodeMessage()
    {
        ...
        if ($fieldValueSession === 1 AND ($donationCode === $donationCodeSession)) {
            return '';
        } elseif (!empty($fieldValueSession) AND !empty($fieldValueHeadSession) && ($donationCode == $donationCodeSession)) {
            EasyFrontendSeoHelper::addHeadData($fieldValueHeadSession);
        ...
        if ($host == 'localhost') {
            $fieldValue = '<div class="' . EasyFrontendSeoHelper::randomClassName($session) . '">' . JText::_('KR_DONATION_CODE_CHECK_DEFAULT_LOCALHOST') . '</div>';
            ...
            $donationCodeCheck = EasyFrontendSeoHelper::getDonationCodeStatus($host, $donationCode);

            if ($donationCodeCheck !== 1) {
                $fieldValue = '<div class="' . EasyFrontendSeoHelper::randomClassName($session) . '">' . JText::sprintf('KR_DONATION_CODE_CHECK_DEFAULT', $host) . '</div>';
    ...
    private static function randomClassName($session)
    {
        ...
        EasyFrontendSeoHelper::addHeadData($headData);
